...
result=$(aws ec2 run-instances \
    ...
    --user-data file://$(pwd)/test-gpudata.yaml)

instanceid=$(echo $result | jq .Instances\[0\].InstanceId | sed 's/"//g')
echo "Instance: $instanceid"
aws ec2 create-tags --region $region --resources $instanceid --tags "Key=Name,Value=Taupage AMI GPU Test, Key=Version,Value=$TAUPAGE_VERSION"
...
for testinstance_type in $testinstance_types; do
	echo "Running AMI tests for $AMI_ID with instance type $testinstance_type"

	# load volume testing definitions - we need a random for every run
	. volume_testing.sh
	...
	result=$(aws ec2 run-instances \
	    ...
	    --image-id $AMI_ID \
	    --count 1 \
	    --instance-type $testinstance_type \
	    --associate-public-ip-address \
	    --key-name $keypair \
	    --security-group-ids $security_group \
	    --subnet-id $subnet \
